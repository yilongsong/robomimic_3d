...
def calculate_quaternion(
    normalized_target_orientation, pos, original_orientation=np.array([0, 0, -1])
):
    normalized_x_y = np.array([pos[0], pos[1]]) / np.linalg.norm([pos[0], pos[1]])
    horizontal_rotation_angle = (
        get_angle_from_sin_cos(normalized_x_y[1], normalized_x_y[0]) + np.pi / 2
    ) % (2 * np.pi)
...
def generate_camera_pos_and_quat(args):
    ...
    normalized_target_vectors = np.array(
        [
            target_vectors[i] / np.linalg.norm(target_vectors[i])
            for i in range(num_cameras)
        ]
    )
    ...
    quats = np.array(
        [
            calculate_quaternion(normalized_target_vectors[i], pos[i])
            for i in range(num_cameras)
        ]
    )
...
def extract_trajectory(args, env_meta, initial_state, states, actions, done_mode=1):
    ...
    for custom_camera_name in args.custom_camera_names:
        env.depthminmax.add(
            custom_camera_name + "_depth", [0.1, args.camera_sphere_radius + 1.0]
        )
    ...
    env.reset()
    insert_index = find_index_to_add_camera(
        initial_state["model"], camera_string='<camera name="agentview'
    )
    ...
    for camera in args.custom_camera_names:
        ...
        initial_state["model"] = (
            initial_state["model"][:insert_index]
            + new_cameras_xml
            + initial_state["model"][insert_index:]
        )
...
def generate_obs_for_dataset_parallel(args):
    ...
    for i in range(start_ind, len(demos), num_workers):
        ...
        inputs = [
            (args, env_meta, initial_state_list[j], states_list[j], actions_list[j])
        ...
        for j in range(len(current_demos)):
            ...
            f_out.flush()
            print(
                "{}: wrote {} transitions to group {}".format(
                    ep, ep_data_grp.attrs["num_samples"], ep
                )
            )
        del inputs
        del trajs
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_xml", type=str, help="path to the environment xml file")
    parser.add_argument(
        "--output_file",
        type=str,
    ...
    parser.add_argument(
        "--num_demos", type=int, default=1, help="number of demos to process"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
